...
dat.t <- dat
names(dat.t)
# Store classe column in an external variable for future use
classe <- dat.t$classe
# The columns 1 - 7 are just bookkeeping
dat.t <- dat.t[, 8:size[2]]
size <- dim(dat.t)
```
Now we have `r size[2]` variable which is still too large number. We can reduce number of variables by exploring the variance of each. The near- or zero variance variables are not useful in most models.
...
The value of Accuracy is very close to the cross-validation result, that means that we successfully avoided overfitting and can try "real-world" data. At first, we should repeat all of the transformations.
